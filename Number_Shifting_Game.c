    ...
    system("cls"); //to clear screen
     ...
     while(1)
     {
         ...
         while (!winner(arr)) //checking for winning situation
         {
             ...
             int key = readEnteredKey(); //this will return ascii code of user entered
...
int shiftRight(int arr[][4])
{
    ...
    return 1; // shifting Right success
}
...
int shiftLeft(int arr[][4])
{
    ...
    swap(&arr[i][j], &arr[i][j + 1]);
    return 1; // shifting left success
}
...
void gameRule(int arr[][4])
{
    ...
    printf(ANSI_COLOR_RED "                      RULE OF THIS GAME            \n"
           ...
           printf(ANSI_COLOR_RED "4.winning situation : " ANSI_COLOR_RESET);
           ...
           printf("\nSo Try to win minimum no of move \n");
